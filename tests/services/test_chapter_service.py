...
class TestSelectChapterByProjectId:
    def test_select_chapter_by_project_id(self, service_with_mock):
        result = service_with_mock.select_chapter_by_project_id(1)
        ...
        service_with_mock.chapter_repo.select_chapter_by_project_id.assert_called_once_with(1)
...
class TestUpdateChaptersStatus:
    def test_update_chapters_status(self, service_with_mock):
        chapters = [
            Chapter(id=1, project_id=1, chapter_number=1, start_page=1, end_page=5, status=ChapterStatus.not_started),
        ...
        service_with_mock.update_chapters_status(chapters, new_status)

        assert chapters[0].status == new_status
        assert chapters[1].status == new_status
        assert service_with_mock.chapter_repo.update.call_count == 2


class TestUpdateChapterExtractedText:
    def test_update_chapter_extracted_text(self, service_with_mock):
        chapters = [
        ...
        service_with_mock.update_chapter_extracted_text(chapters, ocr_results)
...
class TestUpdateChapterScript:
    def test_update_chapter_script(self, service_with_mock):
        chapters = [
        ...
        service_with_mock.update_chapter_script(chapters, script_results)
...
class TestUpdateChapterScriptFileCount:
    def test_update_chapter_script_file_count(self, service_with_mock):
        chapters = [
        ...
        service_with_mock.update_chapter_script_file_count(chapters, tts_results)
...
class TestCreateChapters:
    ...
    def test_create_chapters(self, service_with_mock):
        project_id = 1
        chapters = [
        ...
        result = service_with_mock.create_chapters(project_id, chapters)
